...
const GallerySection = () => {
  ...
  const fetchAndLoadImages = useCallback(async () => {
    try {
      ...
      const formattedImages = data.map((image) => ({
        ...image,
        photoSrc: `data:image/jpeg;base64,${image.photo}`,
      }));
  ...
  const handleDelete = async (imageId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setNotification({
          type: "error",
          message: "Failed to delete image: " + data.error,
        });
      }
    } catch (error) {
  ...
  {images.length === 0 ? (
    <div className="text-white text-center">
      No images found. Start uploading to build your cosmic collection!
    </div>
  ) : (
    <motion.div 
      initial={{ opacity: 0 }}
        ...
        >
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-0 hover:opacity-100 transition-opacity duration-300">
            <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
              <h3 className="text-xl font-semibold font-['Orbitron'] mb-3">
                {image.title}
              </h3>
      ...
      >
        ...
        <div className="flex justify-between mt-6">
          ...
          <motion.button
            ...
            onClick={(e) => {
              e.stopPropagation();
              handleSave(selectedImage.photoSrc);
            }}
          >
            Save
          </motion.button>
